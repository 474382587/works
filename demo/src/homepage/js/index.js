...
function updatePosts(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    // ${arr[i].user_id}
    li.innerHTML = `<a href='./src/blog/post.html?topic_id=${
      arr[i].topic_id
    }&user_id=${arr[i].user_id}'><div class="left"><p>${
      arr[i].nick_name
    } <span>${calculateTime(arr[i].modify_time)}</span></p>${
      arr[i].title
    }</div><div class="right"><span><img src="./static/img/comment-alt.svg" alt="" height="20px">${
      arr[i].comment_num
    }</span><span><img src="./static/img/thumbs-up.svg" alt="" height="20px">${
      arr[i].like_num
    }</span></div></a>`
...
function requestPosts(type, pageNum) {
  ...
  axios({
    method: 'post',
    url:
      'https://cors-anywhere.herokuapp.com/http://www.ftusix.com/static/data/topicList.php',
    params: {
      type: type,
      sort: 'new',
      page: pageNum + 1,
