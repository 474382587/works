...
document.addEventListener('submit', e => {
  ...
  axios({
    method: 'post',
    url:
      'https://cors-anywhere.herokuapp.com/http://www.ftusix.com/static/data/login.php',
    data: {
      mobile: loginNumber,
      pwd: loginPassword
    }
    ...
    .then(function(response) {
      ...
      if (response.data.status === 0) {
        ...
        if (saveCookie) {
          console.log(
            'Use Cookie' + ' token = ' + token + ' userid = ' + userId
          )
          ...
          document.cookie =
            ...
            relPathToAbs('../../')
          document.cookie =
            'token' + '=' + encodeURI(token) + ';path=' + relPathToAbs('../../')
        }
