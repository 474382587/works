...
document.addEventListener('submit', e => {
  if (!validatePhone(celNum.value)) {
    ...
    !validateVeriCode(veriCode.value, verificationCode.replace(/\s/, ''))
  ) {
...
function register(number, passcode, smsCode) {
  axios
    .post(
      'https://cors-anywhere.herokuapp.com/http://www.ftusix.com/static/data/register.php',
      {
        mobile: number,
        pwd: passcode,
        sms_code: smsCode
...
function getMsgCode(number) {
  axios
    .post(
      'https://cors-anywhere.herokuapp.com/http://www.ftusix.com/static/data/sendsms.php',
      {
        mobile: number
      }
    )
...
function validatePassword(first, second) {
  return first.match(/^[0-9A-Za-z]{6,10}$/)
    ? first === second
      ? true
      : false
    : false
